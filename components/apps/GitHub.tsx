...
export default function GitHub({ isDarkMode = true }: GitHubProps) {
  ...
  useEffect(() => {
    if (!opened.current) {
      opened.current = true
      window.open("https://github.com/erikleon", "_blank")
    }
  }, [])
  ...
  return (
    <div className={`flex h-full items-center justify-center ${isDarkMode ? "bg-gray-900 text-white" : "bg-white text-gray-800"}`}>
      <div className="text-center">
        <svg viewBox="0 0 24 24" fill="currentColor" className="mx-auto mb-4 size-16">
        ...
        <h2 className="text-xl font-semibold">Opening GitHub...</h2>
      </div>
    </div>
  )
}
